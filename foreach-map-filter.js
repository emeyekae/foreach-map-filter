...
function doubleValues(arr){return arr.map(function(value){
    return value *2 ;
});
}
...
Examples:
    onlyEvfenValues([1,2,3]) // [2]
    onlyEvenValues([5,1,2,3,10]) // [2,10]
*/

function onlyEvenValues(arr){return arr.filter(function(value){
    return !(value % 2);
})
}
...
function showFirstAndLast(arr){ let arrOut = []; for (let i=0; i < arr.length ;i++){
     arrOut.push(arr[i].slice(0,1) + arr[i].slice(-1))
    }
    return arrOut;
}
...
function doubleValuesWithMap(arr){return arr.map(function(value,index,array){
    return  value*2;
}
    )
}
...
function valTimesIndex(arr){return arr.map(function(value, index ,array){
    return value * index;
}
)
}
...
a new array with the value of the key with a name of "first" and the value of a key with 
...
function findInObj(arr, key, searchValue) {
    return  arr.filter(function(value,index,arr){
        return value[key] !== undefined && value[key]==searchValue
           } 
         )[0]
      }
...
with all of the vowels (both uppercased and lowercased) removed. Every character in the 
